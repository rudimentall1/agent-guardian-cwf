  ...
  return async (req: IncomingMessage, res: ServerResponse) => {
    try {
      ...
      if(req.method==="POST" && req.url==="/v1/agent/demo/security-demo"){
        ...
        const toolRequest:ToolRequest={agent:demoState.agent,wallet:demoState.wallet,tool:"demo",action:"ping",args:[123],value:0n,nonce,deadline,policyHash:demoState.policyHash};
        const resolved=resolveToolRequest(definition,toolRequest);
        const intent=resolved.intent;
        push("AGENT_REQUEST","complete",{tool:toolRequest.tool,action:toolRequest.action,args:toolRequest.args});
        push("TOOL_DEFINITION","resolved",{target:intent.target,selector:intent.data.slice(0,10),calldataBytes:(intent.data.length-2)/2});
        const risk=await assessRisk({intent,chainId:context.chainId},context.riskProviders??[]);
        ...
        const attackRequest:ToolRequest={...toolRequest,nonce:attackNonce,deadline:BigInt(Math.floor(Date.now()/1000)+900)};
        const attackResolved=resolveToolRequest(definition,attackRequest);
        const signedAttack=attackResolved.intent;
        const attackSignature=await signIntent(context.demoAgentSigner,signedAttack,context.chainId,demoState.guard);
        const tamperedIntent={...signedAttack,data:ethers.concat([signedAttack.data.slice(0,10),ethers.zeroPadValue(ethers.toBeHex(999),32)])};
